...
public class GenericController {
	...
	public ResponseEntity<?> autoAllocationOfLeads(@RequestBody LeadMapReq req)
	   ...
	   List<LeadStageRefEntity> getLeadStagesById(@PathVariable(name="leadid") String id) throws DynamicFormsServiceException{
		...
		@RequestMapping(value = "/uploadBulkUploadForUnionTerrotry", method = { RequestMethod.POST })
		public ResponseEntity<?> uploadBulkUploadForInsuranceDetails(@RequestPart("file") MultipartFile bulkExcel,
			...
			try {
				if (null != empId && null != orgId && !bulkExcel.isEmpty()) {

					response = genericService.processBulkExcelForUnionTerritory(String.valueOf(empId),String.valueOf(orgId),bulkExcel);
				}
			} catch (Exception e) {
